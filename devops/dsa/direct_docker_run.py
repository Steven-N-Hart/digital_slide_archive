...
def run(task, **kwargs):
    """Wraps docker_run to support direct mount volumes."""

    pull_image = kwargs.get('pull_image')
    if pull_image == 'if-not-present':
        kwargs['pull_image'] = not _has_image(kwargs['image'])

    # Pass the worker container's bind mounts into the spawned CLI container
    # via volumes_from.  This makes host paths such as /Export/Shared available
    # inside the container without having to enumerate them individually.
    try:
        container_id = os.uname().nodename
        kwargs.setdefault('volumes_from', [container_id])
    except Exception:
        pass

    # Pass all host GPUs into the spawned CLI container so TRIDENT can use CUDA.
    try:
        import docker as _docker
        kwargs.setdefault('device_requests', [
    ...
    TRIDENT_STAGING = '/Export/Shared/DSA/TRIDENT'
    if os.path.isdir(TRIDENT_STAGING):
        host_path = _adjust_docker_path(TRIDENT_STAGING)
        staging_vol = BindMountVolume(host_path, TRIDENT_STAGING, mode='rw')
        vols = kwargs.setdefault('volumes', [])
        if isinstance(vols, list):
            vols.append(staging_vol)
        else:
            vols.update(staging_vol._repr_json_())
